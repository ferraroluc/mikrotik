...
/ip ipsec identity add auth-method=digital-signature certificate=server generate-policy=port-strict match-by=certificate mode-config=modeconfig-ikev2-$USER_TYPE peer=peer-wan policy-template-group=group-ikev2 remote-certificate=$NAME remote-id=user-fqdn:$NAME@DOMAIN.com
EOF
)
